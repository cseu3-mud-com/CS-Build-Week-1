...
def generateRoomDescription():
    ...
    nm4 = ["large", "small", "massive", "grand", "modest", "scanty", "narrow"]
    nm5 = ["broken", "clammy", "crumbling", "damp", "dank", "dark", "deteriorated", "dusty", "filthy", "foggy", "grimy", "humid", "putrid", "ragged", "shady", "timeworn", "weary", "worn"]
    nm6 = ["ash", "bat droppings", "broken pottery", "broken stone", "cobwebs", "crawling insects", "dead insects", "dead vermin", "dirt", "large bones", "moss", "puddles of water", "rat droppings", "remains", "roots", "rubble", "small bones"]
    ...
    nm8 = ["is a single path", "are two paths, you take the right", "are two paths, but the right is a dead end", "are two paths, you take the left", "are two paths, but the left is a dead end", "are three paths, you take the right", "are three paths, you take the left", "are three paths, you take the middle"]
    nm9 = ["downwards", "onwards", "passed broken and pillaged tombs", "passed collapsed rooms and pillaged treasuries", "passed countless other pathways", "passed countless rooms", "passed long lost rooms and tombs", "passed lost treasuries, unknown rooms and armories", "passed pillaged rooms", "passed several empty rooms"]
    nm10 = ["clammy", "crumbled", "damp", "dank", "dark", "dusty", "filthy", "foggy", "ghastly", "ghostly", "grimy", "humid", "putrid", "ragged", "shady", "timeworn", "weary", "worn"]
    ...
    nm12 = ["advance carefully", "carefully continue", "cautiously proceed", "continue", "move", "press", "proceed", "slowly march", "slowly move", "tread"]
    nm13 = ["darkness", "depths", "expanse", "mysteries", "secret passages", "secrets", "shadows"]
    nm14 = ["a few more passages", "a few more rooms and passages", "countless passages", "dozens of similar rooms and passages", "many different passages", "many rooms and passages", "constious different rooms and countless passages", "constious passages"]
    nm15 = ["A big", "A grand", "A huge", "A large", "A massive", "A mysterious", "A tall", "A thick", "A vast", "A wide", "An enormous", "An immense", "An ominous"]
    ...
    nm18 = ["Ash and soot is", "Countless odd symbols are", "Countless runes are", "Dire warning messages are", "Dried blood splatters are", "Intricate carvings are", "Large claw marks are", "Messages in strange languages are", "Ominous symbols are", "Strange writing is", "constious odd symbols are"]
    nm19 = ["did something just move behind this door?", "you're sure you saw a shadow under the cracks of the door.", "did the door just change its appearance?", "what was that sound?", "you're pretty sure you're being watched.", "was that a growl coming from behind the door?", "did somebody just knock on the door?", "you hear the faint sound of footsteps behind you.", "is that a scratching sound coming from behind the door?", "you think you can hear a whisper coming from behind the door.", "light's coming through the gap below the door.", "you hear a loud bang in the distance from which you came.", "you hear a faint laugh coming from behind the door.", "suddenly the door slowly opens on its own.", "something just grabbed your shoulder."]
    nm20 = ["bleak", "dark", "dire", "eerie", "foggy", "gloomy", "grim", "misty", "murky", "overcast", "shadowy", "shady", "sinister", "somber"]
    nm21 = ["aged", "battered", "busted", "decayed", "demolished", "destroyed", "deteriorated", "forgotten", "frayed", "long lost", "pillaged", "tattered", "wasted", "weathered", "worn", "worn down"]
    nm22 = ["absorbed", "butchered", "claimed", "consumed", "defaced", "desolated", "devoured", "dismantled", "drained", "eaten", "maimed", "mutilated", "ravaged", "ravished", "spoiled", "taken", "wiped out", "wrecked"]

    rnd1 = randint(0, len(nm1) - 1)
    rnd2 = randint(0, len(nm2) - 1)
    rnd3 = randint(0, len(nm3) - 1)
    rnd4 = randint(0, len(nm4) - 1)
    rnd5 = randint(0, len(nm5) - 1)
    rnd6 = randint(0, len(nm6) - 1)
    rnd6a = randint(0, len(nm6) - 1)
    rnd6b = randint(0, len(nm6) - 1)
    rnd7 = randint(0, len(nm7) - 1)
    rnd8 = randint(0, len(nm8) - 1)
    rnd9 = randint(0, len(nm9) - 1)
    rnd10 = randint(0, len(nm10) - 1)
    rnd11 = randint(0, len(nm11) - 1)
    rnd12 = randint(0, len(nm12) - 1)
    rnd13 = randint(0, len(nm13) - 1)
    rnd14 = randint(0, len(nm14) - 1)
    rnd15 = randint(0, len(nm15) - 1)
    rnd16 = randint(0, len(nm16) - 1)
    rnd17 = randint(0, len(nm17) - 1)
    rnd18 = randint(0, len(nm18) - 1)
    rnd19 = randint(0, len(nm19) - 1)
    rnd20 = randint(0, len(nm20) - 1)
    rnd21 = randint(0, len(nm21) - 1)
    rnd22 = randint(0, len(nm22) - 1)

    title = nm1[rnd1] + " " + nm2[rnd2] + " in a " + nm20[rnd20] + " " + nm3[rnd3]
    name = title + " marks the entrance to this dungeon. Beyond the " + nm2[rnd2] + " lies a " + nm4[rnd4] + ", " + nm5[rnd5] + " room. It's covered in " + nm6[rnd6] + ", " + nm6[rnd6a] + " and " + nm6[rnd6b] + "."
    name2 = "Your torch allows you to see " + nm7[rnd7] + ", " + nm21[rnd21] + " and " + nm22[rnd22] + " by time itself."
